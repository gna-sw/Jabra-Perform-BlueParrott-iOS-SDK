...
- (void)viewDidLoad {
    ...
    [self addStatus:log];
    [self checkRemoteLogging];
    
    // Whenever the app is brought to the foreground, log the current state of the remote logging setting:
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(checkRemoteLogging)
                                                 name:UIApplicationWillEnterForegroundNotification
                                               object:nil];
}

- (void) checkRemoteLogging {
    BPHeadset.remoteLogging = [[NSUserDefaults standardUserDefaults] boolForKey:@"remote_logging"];
    NSString *log = [NSString stringWithFormat:@"Remote Logging Enabled: %@",BPHeadset.remoteLogging ? @"YES" : @"NO"];
    [self addStatus:log];
